...
# @url https://core.telegram.org/bots/api#chatmemberadministrator# ['Represents a chat member that has some additional privileges.']
class ChatMemberAdministrator(DefaultType):

    
	# @var status The member's status in the chat, always "administrator"
	@property
	...
	def user(self) -> User:
		return self._d["user"]
	# @var can_be_edited True, if the bot is allowed to edit administrator privileges of that user
	@property
