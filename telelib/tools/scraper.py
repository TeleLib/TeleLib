...
class Scraper:
    ...
    def __init__(self):
        ...
        self.APPROVED_NO_SUBTYPES = {
            "InputFile"
        }
        self.APPROVED_MULTI_RETURNS = [
            ['Message', 'Boolean']
        ]
    ...
    def get_fields(
        ...
        for tr in body.find_all("tr"):
            ...
            if curr_type == self.TYPES and len(children) == 3:
                desc = self.clean_tg_field_description(children[2], url)
                fields.append(
                    {
                        "name": children[0].get_text(),
                        "types": self.clean_tg_type(children[1].get_text()),
                ...
                fields.append(
                    {
                        ...
                        "description":
                            self.clean_tg_field_description(children[3], url)
                    }
                )
    ...
    def extract_return_type(
        ...
        if array_match:
            ret = self.clean_tg_type(array_match.group(1))
    ...
    def clean_tg_field_description(self, t: Tag, url: str) -> str:
        return " ".join(self.clean_tg_description(t, url))

    def clean_tg_description(self, t: Tag, url: str) -> list[str]:
    ...
    def get_proper_type(self, t: str) -> str:
        if t == "Messages":
            return "Message"
    ...
    def clean_tg_type(self, t: str) -> list[str]:
        pref = ""
        ...
        return [pref + self.get_proper_type(x) for x in fixed_commas]
    ...
    def get_subtypes(
        ...
        for li in x.find_all("li"):
            list_contents.extend(self.clean_tg_description(li, url))
        ...
        for x in list(dev_rules.children):
            ...
            if x.name == "p":
                items[curr_type][curr_name].setdefault(
                    "description", []).extend(
                        self.clean_tg_description(x, url)
                )

            if x.name == "table":
                self.get_fields(curr_name, curr_type, x, items, url)

            if x.name == "ul":
                self.get_subtypes(curr_name, curr_type, x, items, url)

            if curr_type == self.METHODS and \
    ...
    def __verify_types(self, items: dict) -> bool:
        issue_found = False
        ...
        for type_name, values in items[self.TYPES].items():
            ...
            for param in fields:
                ...
                for field_type_name in field_types:
                    ...
                    if field_type_name not in items[self.TYPES] \
    ...
    def __verify_methods(self, items: dict) -> bool:
        issue_found = False
        ...
        for method, values in items[self.METHODS].items():
            ...
            for ret in values.get("returns", []):
                ...
                if ret not in items[self.TYPES] \
    ...
    def run(self, filename, indent=4):
