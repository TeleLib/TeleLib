...
# reasonable amount of attempts. Returns True on success., If
# you'd like to make sure that the webhook was set by you, you
# can specify secret data in the parameter secret_token. If
# specified, the request will contain a header "X-Telegram-
...
# MB in size, this limit may be changed in the future., For
...
# support MPEG4 videos (other formats may be sent as
# Document). On success, the sent Message is returned. Bots
# can currently send video files of up to 50 MB in size, this
...
# As of v.4.0, Telegram clients support rounded square MPEG4
...
# success., We only recommend using this method when a
# response from the bot will take a noticeable amount of time
# to arrive.
...
# Use this method to get basic information about a file and
# prepare it for downloading. For the moment, bots can
# download files of up to 20MB in size. On success, a File
# object is returned. The file can then be downloaded via the
# link https://api.telegram.org/file/bot<token>/<file_path>,
# where <file_path> is taken from the response. It is
# guaranteed that the link will be valid for at least 1 hour.
# When the link expires, a new one can be requested by calling
# getFile again., Note: This function may not preserve the
# original file name and MIME type. You should save the file's
# MIME type and name (if available) when the File object is
# received.
...
# Use this method to get the current value of the bot's menu
...
# groups or channels. These rights will be suggested to users,
# but they are are free to modify the list before adding the
...
# rights of the bot. Returns ChatAdministratorRights on
# success.
...
# can delete incoming messages in private chats., - Bots
# granted can_post_messages permissions can delete outgoing
# messages in channels., - If the bot is an administrator of a
...
# success, True is returned., No more than 50 results per
# query are allowed.
...
# user to the chat from which the query originated. On
# success, a SentWebAppMessage object is returned.
...
# Use this method to create a link for an invoice. Returns the
...
# must change). Returns True on success., Use this if the data
# submitted by the user doesn't satisfy the standards your
# service requires for any reason. For example, if a birthday
# date seems invalid, a submitted document is blurry, a scan
# shows evidence of tampering, etc. Supply some details in the
# error message to make sure the user knows how to correct the
# issues.
